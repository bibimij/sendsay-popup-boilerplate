...
class Sendsay
{
	 ...
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%9F%D0%BE%D0%B4%D1%82%D0%B2%D0%B5%D1%80%D0%B4%D0%B8%D1%82%D1%8C-%D1%80%D0%B5%D0%B3%D0%B8%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8E][Документация]
	 *
	 * @param  string  емэйл подписчика
	 ...
	 * @return array
	 */
	public function member_confirm($email, $cookie)
	{
		$this->params = $this->auth+array(
	...
	/**
	 * Массово изменяет данные подписчиков.
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%9C%D0%B0%D1%81%D1%81%D0%BE%D0%B2%D0%BE%D0%B5-%D0%B8%D0%B7%D0%BC%D0%B5%D0%BD%D0%B5%D0%BD%D0%B8%D0%B5-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85-%D0%BF%D0%BE%D0%B4%D0%BF%D0%B8%D1%81%D1%87%D0%B8%D0%BA%D0%BE%D0%B2][Документация]
	 *
	 * @param  mixed   список подписчиков (array) или код группы (string)
	 * @param  string  код формата
	 * @param  string  действие с заполненным пунктом анкеты (overwrite — заменить значением из формата | merge — объединить значения | skip — оставить старое значение)
	 * @param  string  действие с незаполненным пунктом анкеты (set — подставить значение из формата | skip — оставить пункт незаполненным)
	 * @param  bool   асинхронность вызова
	 *
	 * @return array
	 */
	public function member_update($data, $format, $has='overwrite', $hasnt='set', $sync=FALSE)
	{
		$this->params = $this->auth+array(
	...
	/**
	 * Возвращает список файлов, приложенных к письму.
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%9F%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-%D1%84%D0%B0%D0%B9%D0%BB%D0%BE%D0%B2-%D0%BF%D1%80%D0%B8%D0%BB%D0%BE%D0%B6%D0%B5%D0%BD%D0%BD%D1%8B%D1%85-%D0%BA-%D0%B2%D1%8B%D0%BF%D1%83%D1%81%D0%BA%D1%83][Документация]
	 *
	 * @param  int  код выпуска
	 *
	 * @return array
	 */
	public function issue_get_attach($id)
	{
		$this->params = $this->auth+array(
	 ...
	 *
	 * @param  array   коды групп; если пустой массив - по всем
	 * @param  mixed   способ возврата результата; тип (response|save) или список получателей (array)
	 * @param  string  формат вывода (csv|xlsx)
	 *
	 * @return array
	...
	/**
	 * Возвращает портрет аудитории по группе.
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%9F%D0%BE%D1%80%D1%82%D1%80%D0%B5%D1%82-%D0%B0%D1%83%D0%B4%D0%B8%D1%82%D0%BE%D1%80%D0%B8%D0%B8][Документация]
	 *
	 * @param  string  код группы; если пусто - по всем
	 * @param  bool    включить никем невыбранные ответы
	 * @param  bool    включить вопросы без ответа
	 * @param  mixed   способ возврата результата; тип (response|save) или список получателей (array)
	 * @param  string  формат вывода (csv|xlsx)
	 *
	 * @return array
	 */
	public function stat_group_portrait($group=NULL, $empty_resp=FALSE, $empty_quest=FALSE, $result='save', $format='csv')
	{
		$this->params = $this->auth+array(
			'action'      => 'stat.group.portrait',
			'empty_resp'  => $empty_resp,
			'empty_quest' => $empty_quest,
			'result'      => is_array($result) ? 'email' : $result
		);
	 ...
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA-%D1%87%D0%B5%D1%80%D0%BD%D0%BE%D0%B2%D0%B8%D0%BA%D0%BE%D0%B2][Документация]
	 *
	 * @return array
	 */
	public function issue_draft_list()
	{
		$this->params = $this->auth+array(
	 ...
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%9F%D1%80%D0%B5%D0%B4%D0%BF%D1%80%D0%BE%D1%81%D0%BC%D0%BE%D1%82%D1%80-%D1%87%D0%B5%D1%80%D0%BD%D0%BE%D0%B2%D0%B8%D0%BA%D0%B0][Документация]
	 *
	 * @param  mixed   код черновика (int) или содержимое черновика (array)
	 * @param  string  емэйл для подстановки данных
	 *
	 * @return array
	 */
	public function issue_draft_preview($id, $email=NULL)
	{
		$this->params = $this->auth+array(
	 ...
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#%D0%9F%D0%BE%D0%BB%D1%83%D1%87%D0%B8%D1%82%D1%8C-%D1%84%D0%B0%D0%B9%D0%BB][Документация]
	 *
	 * @param  string  расположение скачиваемого файла
	 * @param  string  расположение файла итогового
	 * @param  string  домен (image|report)
	 *
	 * @return array
	 */
	public function get_file($from, $to, $domain='image')
	{
		$this->params = $this->auth+array(
			'action'   => 'rfs.file.get',
			'domain'   => $domain,
		...
		{
			file_put_contents($to, base64_decode($result['data']));
		}
	 ...
	 * @return array
	 */
	public function rm($path, $domain='image')
	{
		$this->params = $this->auth+array(
			'action' => strpos(basename($path), '.') === FALSE ? 'rfs.dir.delete' : 'rfs.file.delete',
			'domain' => $domain,
			'path'   => $path
		);
	 ...
	 *
	 * @link  [https://pro.subscribe.ru/API/API.html#C%D0%BF%D0%B8%D1%81%D0%BE%D0%BA-%D1%84%D0%B0%D0%B9%D0%BB%D0%BE%D0%B2][Документация]
	 *
	 * @param  string  путь
	 * @param  string  домен (image|report)
	 *
	 * @return array
	 */
	public function ls($path, $domain='image')
	{
		$this->params = $this->auth+array(
			'action' => 'rfs.list',
			'domain' => $domain,
